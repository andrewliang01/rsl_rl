...
def delta_rule_chunkwise(
    ...
    for start in range(0, q.shape[2], chunk_size):
        ...
        system = system + torch.tril(
            beta_chunk * (k_chunk @ k_chunk.transpose(-1, -2)),
            diagonal=-1,
        )
        errors = torch.linalg.solve_triangular(
            system,
            beta_chunk * (v_chunk - k_chunk @ state.transpose(-1, -2)),
            upper=False,
            unitriangular=True,
        )
        outputs.append(
            q_chunk @ state.transpose(-1, -2)
            + torch.tril(q_chunk @ k_chunk.transpose(-1, -2)) @ errors
        )
        state = state + errors.transpose(-1, -2) @ k_chunk
...
class DeltaNetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, packed: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if length == 1:
            ...
            result, state = delta_rule_chunkwise(q, k, v, beta, state, self.chunk_size)
